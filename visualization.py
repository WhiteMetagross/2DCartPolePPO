#This program visualizes the training progress of a PPO agent.
#It can be used to visualize the training progress of a trained agent and to create plots.
...
#This class visualizes the training progress of a PPO agent.
class PPOTrainingVisualizer:

    #This function initializes the PPOTrainingVisualizer class.
    def __init__(self, log_dir="logs/"):
        self.log_dir = log_dir
        self.data = {}
        plt.style.use('seaborn-v0_8')
    
    #This function loads the training data from a JSON file or parses a log file.
    ...
    #This function parses the training log file to extract metrics.
    ...
    #This function creates a plot of the training progress.
    def plot_training_progress(self, save_plots=True, show_plots=True):
        ...
        if 'reward_history' in self.data and self.data['reward_history']:
            ...
            steps_all = self.data.get('steps', list(range(len(rewards))))
            x_vals = steps_all
            x_label = 'Training Steps'

            #Align x/y: if steps were logged at a different cadence, prefer eval_steps when it matches
            if len(steps_all) != len(rewards):
                eval_steps = self.data.get('eval_steps')
                if eval_steps is not None and len(eval_steps) == len(rewards):
                    x_vals = eval_steps
                    x_label = 'Update Number'
                else:
                    min_len = min(len(steps_all), len(rewards))
                    x_vals = steps_all[:min_len]
                    rewards = rewards[:min_len]

            ax1.plot(x_vals, rewards, alpha=0.7, color='blue', label='Training Reward')

            #Add rolling average.
            if len(rewards) > 10:
                window = min(50, len(rewards) // 10)
                rolling_avg = np.convolve(rewards, np.ones(window)/window, mode='valid')
                ax1.plot(x_vals[window-1:], rolling_avg, color='red', linewidth=2, label=f'{window}-Step Average')
        ...
        if 'eval_rewards' in self.data and self.data['eval_rewards']:
            ...
            eval_steps = self.data.get('eval_steps', list(range(len(eval_rewards))))
            min_len = min(len(eval_steps), len(eval_rewards))
            ax2.plot(eval_steps[:min_len], eval_rewards[:min_len], 'o-', color='green', linewidth=2, markersize=6)
        ...
        if 'policy_losses' in self.data and self.data['policy_losses']:
            ...
            steps_all = self.data.get('steps', list(range(len(policy_losses))))
            min_len = min(len(steps_all), len(policy_losses))
            ax3.plot(steps_all[:min_len], policy_losses[:min_len], alpha=0.7, color='orange')
        ...
        if 'value_losses' in self.data and self.data['value_losses']:
            ...
            steps_all = self.data.get('steps', list(range(len(value_losses))))
            min_len = min(len(steps_all), len(value_losses))
            ax4.plot(steps_all[:min_len], value_losses[:min_len], alpha=0.7, color='purple')
        ...
        if save_plots:
            ...
            print("Training progress plot saved as 'ppo_training_progress.png'")
    ...
    #This function creates a plot of the reward analysis.
    def plot_reward_analysis(self, save_plots=True, show_plots=True):
        ...
        fig.suptitle('PPO Reward Analysis', fontsize=16, fontweight='bold')
        ...
        if save_plots:
            plt.savefig('ppo_reward_analysis.png', dpi=300, bbox_inches='tight')
            print("Reward analysis plot saved as 'Ppo_reward_analysis.png'")
...
#This function is the main entry point for the visualization program.
def main():
    ...
    visualizer = PPOTrainingVisualizer(log_dir=args.log_dir)
